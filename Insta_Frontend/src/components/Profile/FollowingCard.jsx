...
const FollowingCard = ({ item, i,flag }) => {
  const {user}=useSelector(state=>state.User)
  const [following, setFollowing] = useState(user.following.find(({ user }) =>user===item.user._id)?true:false);
  ...
  return (
    <>
      ...
      >
        <div className="followingCard1">
          <img src={item.user.profile?item.user.profile.url:ProfileImg} alt="" />
        </div>
        <div className="followingCard2">
          <p>{item.user.username}</p>
          <p>{item.user.name}</p>
        </div>
        <div className="followingCard3">
          <button
